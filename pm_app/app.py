...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Tabs([
            dcc.Tab(label="Overview", children=render_overview_tab()),
...
@app.callback(
    ...
    Input("tbl-overview", "data"),
    prevent_initial_call=True,
)
def _sync_overview_store(table_rows):
    return table_rows


@app.callback(
    Output("lbl-overview-status", "children"),
    Input("btn-save-overview", "n_clicks"),
    State("store-overview", "data"),
    prevent_initial_call=True,
)
def _save_overview(n_clicks, rows):
    d = rows_to_overview(rows)
    save_overview(d)
